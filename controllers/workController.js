...
class WorkController {
    ...
    async getAll(req, res){
        ...
        let devices;
        if (!brandId && !typeId){
            devices = await Work.findAndCountAll({where: {
        ...
        if (!brandId && typeId){
            devices = await Work.findAndCountAll({where: {typeId}, limit, offset})

        } 
        if (!typeId && brandId){
            devices = await Work.findAndCountAll({where: {brandId}, limit, offset})
        } 
        if (brandId && typeId){
            devices = await Work.findAndCountAll({where: {brandId, typeId}, limit, offset})
        }
    ...
    async getAllWithParams(req, res){
        ...
        upcomingWorks = await Work.findAll({where: {
            nextWorkDate: { [Op.between]: [new Date(today), nextDate] }
        }})
        overdueWorks = await Work.findAll({where: {
